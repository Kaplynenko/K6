...
export class HttpClient {
    ...
    request(method, path, body) {
        ...
        const response = body ? http[method](url, requestBody, this.params) : http[method](url, this.params);

        const err = response.error_code ? `code: ${response.error_code} ${response.error}` : '';

        console.log(`[http] ${response.status_text} ${methodWithUrl} ${err}`);

        check(response, {
            [`[http] ${methodWithUrl} is OK`]: (r) => r.status >= 200 && r.status < 300,
        });
        if(err) {
            return {};
        }
